...
export async function login({ email, password, mfaCode, logLevel }: {
  ...
  const userinfo = {
    name: `${user.accountInfo.address.firstName} ${user.accountInfo.address.lastName}`,
    email: email,
    passwordToken: user.passwordToken,
    directoryServicesIdentifier: user.dsPersonId,
    cookies: StoreClient.storeReq.cookieJar.getSetCookieStringsSync('https://apple.com', { allPaths: true }),
  };
